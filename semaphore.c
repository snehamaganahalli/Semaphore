...
/* NOTE: Dont think there is no use of the semaphore, since the child is executed first and then the parent is executed because of pthread_join API
   ...
   IMPORTANT: Using semaphore for a global variable between the parent and child created by fork is meaningless.
   Because the child and parent will share the copy of the memeory pages. And each has their own copy. Threfore single resource
   is not shared between the 2 process.  So there no need to use semaphore.
